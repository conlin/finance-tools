...
class BankCard
{
    ...
    public static function info($cardNo)
    {
        ...
        $url = sprintf($url, $cardNo);
        $result = self::getCurl($url);
        $result = json_decode($result);
        if (!isset($result->validated)) {
            throw new \RuntimeException('数据异常：' . $url . " | 错误：" . curl_error($ch));
        }
    ...
    private static function getCurl($url)
    {
        ...
        if (curl_errno($ch) != 0 || empty($result)) {
            throw new \RuntimeException('无法访问：' . $url . " | 错误：" . curl_error($ch));
        }
        curl_close($ch);
        return $result;
    }
